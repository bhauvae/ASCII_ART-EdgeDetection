...
def apply_opencv_sharpen(image_np, sharp_strength=1.0):
    ...
    kernel_sharpening[1, 1] = kernel_center + (1 - sharp_strength) * (
        4 if sharp_strength > 1 else 1
    )
...
def apply_beam_bloom(image_np, scanline_frequency=1, bloom_intensity=0.5):
    ...
    for y in range(height):
        if (
            y % scanline_frequency == 0
        ):  # Apply effect on every nth line based on scanline frequency
            ...
            image_np[y] = np.clip(image_np[y] * bloom_factor[:, None], 0, 255).astype(
                np.uint8
            )
    return image_np
...
def apply_halation_effect(image_np, sigma_halation=5, halation_opacity=0.5):
    ...
    blended_image = cv2.addWeighted(
        image_np, 1 - halation_opacity, halation_layer, halation_opacity, 0
    )
...
def stretch_image(original_np, width, height):
    ...
    for y in range(height):
        for x in range(width):
            ...
            stretched_image[y * 2 + 1, x * 2 + 1] = (
                original_pixel  # Stretch both directions
            )
    return stretched_image


def apply_scanline_modulation(
    stretched_image, frequency=1, amplitude=0.5, phase_shift=0
):
    """
    ...
    y_positions = np.arange(
        height // 2
    )  # Only apply to every other line to simulate scanlines
    sine_wave = (
        np.sin(2 * np.pi * frequency * (y_positions / (height // 2)) + phase_shift)
        * amplitude
        + 1
        - amplitude
    ) / 2
    for y in range(height // 2):
        ...
        stretched_image[y * 2 + 1] = (
            stretched_image[y * 2 + 1] * mod_intensity
        ).astype(np.uint8)
    return stretched_image
...
def process_image(image_path, output_dir="output", affix="-crt", **kwargs):
    ...
    stretched_image = cv2.resize(
        original_np, (width * 2, height * 2), interpolation=cv2.INTER_NEAREST
    )
    blurred_image = apply_phosphor_decay(stretched_image, sigma=kwargs.get("sigma", 2))
    halation_image = apply_halation_effect(
        blurred_image,
        sigma_halation=kwargs.get("sigma_halation", 5),
        halation_opacity=kwargs.get("halation_opacity", 0.5),
    )
    final_image = apply_scanline_modulation(
        halation_image,
        kwargs.get("frequency", 1),
        kwargs.get("amplitude", 0.5),
        kwargs.get("phase_shift", 0),
    )
    final_image_with_bloom = apply_beam_bloom(
        final_image,
        scanline_frequency=kwargs.get("scanline_frequency", 2),
        bloom_intensity=kwargs.get("bloom_intensity", 0.5),
    )
    sharpened_image = apply_opencv_sharpen(
        final_image_with_bloom, sharp_strength=kwargs.get("sharp_strength", 0.1)
    )
    saturated_image = apply_saturation(
        sharpened_image, saturation_factor=kwargs.get("saturation_factor", 1.1)
    )
    gamma_corrected_image = apply_gamma_correction(
        saturated_image, gamma=kwargs.get("gamma", 2.2)
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        )  # Saturation control, 1.0 is no saturation applied.
