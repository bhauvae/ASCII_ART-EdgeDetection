...
def diff_of_gaussiian(image, apply_normalize=True, apply_clahe=True, apply_contrastthresh=True):
    ...
    # Convert image to UMat for OpenCL support
    image_umat = cv2.UMat(image)

    blur1 = cv2.GaussianBlur(image_umat, (k_size1, k_size1), sigma1)
    blur2 = cv2.GaussianBlur(image_umat, (k_size2, k_size2), sigma2)
